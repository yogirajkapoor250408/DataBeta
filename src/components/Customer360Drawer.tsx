...
export const Customer360Drawer: React.FC<Customer360DrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/75 backdrop-blur-xs flex justify-end transition-opacity duration-200">
      <div className="bg-zinc-950 text-white w-full max-w-xl h-full border-l border-zinc-800 shadow-2xl flex flex-col justify-between overflow-hidden animate-slideInRight">
        {/* Drawer Header */}
        <div className="p-6 border-b border-zinc-900 flex items-start justify-between bg-zinc-950">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-rose-600/20 border border-rose-600/40 text-rose-500 font-black text-xl flex items-center justify-center shadow-lg">
              {customer.name.charAt(0).toUpperCase()}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-black text-white">{customer.name}</h2>
                <span className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase">
                  Active Buyer
                </span>
              </div>
              <p className="text-xs text-zinc-400 mt-0.5">Customer 360 Intelligence Dossier</p>
            </div>
          </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-6 text-xs scrollbar-none">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-4 rounded-2xl bg-zinc-900/80 border border-zinc-800 text-center">
              <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Lifetime Value</div>
              <div className="text-base font-black text-rose-500 mt-1">
                {formatCurrency(customer.totalSpent, currency)}
              </div>
            </div>
            <div className="p-4 rounded-2xl bg-zinc-900/80 border border-zinc-800 text-center">
              <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Total Orders</div>
              <div className="text-base font-black text-white mt-1">{customer.orderCount}</div>
            </div>
            <div className="p-4 rounded-2xl bg-zinc-900/80 border border-zinc-800 text-center">
              <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Avg Order Value</div>
          ...
          <div className="p-4 rounded-2xl bg-zinc-900/60 border border-zinc-800 space-y-2.5">
            <h4 className="font-extrabold text-white text-xs uppercase tracking-wider text-zinc-400">Contact Information</h4>
            ...
            {purchasedProducts.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {purchasedProducts.map((p) => (
                  <span
                    key={p}
                    className="px-3 py-1.5 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-200 font-bold text-[11px]"
                  >
                    {p}
                  </span>
                ))}
              ...
              <p className="text-zinc-500 text-xs italic">No explicit product names logged in transactions.</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {customerDeals.length > 0 ? (
                ...
                {customerDeals.map((d) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[10px] text-zinc-400 uppercase font-mono mt-0.5">Stage: {d.stage.replace('_', ' ')}</div>
                    </div>
              ...
              <p className="text-zinc-500 text-xs italic">No active CRM deals linked to this account.</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="border border-zinc-800 rounded-2xl overflow-hidden">
              <table className="w-full text-left text-xs">
                <thead className="bg-zinc-900 text-zinc-400 font-bold uppercase text-[10px] border-b border-zinc-800">
                  <tr>
                  ...
                  {customerTxs.length > 0 ? (
                    customerTxs.slice(0, 10).map((t) => (
                      <tr key={t.id} className="hover:bg-zinc-900/50">
                        <td className="p-3 text-zinc-400 font-mono">{t.dateString}</td>
                        <td className="p-3 font-medium text-white">{t.product || t.category || 'Sale'}</td>
                    ...
                    <tr>
                      <td colSpan={3} className="p-4 text-center text-zinc-500">No transactions recorded</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="p-4 border-t border-zinc-900 bg-zinc-950 text-center">
          <button
            onClick={onClose}
            className="w-full py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white font-extrabold rounded-full border border-zinc-800 transition-colors text-xs"
          >
