    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-950 text-white p-7 rounded-3xl border border-zinc-800 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-2xl font-black text-white tracking-tight">Active Sales Pipeline</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-zinc-950 p-5 rounded-3xl border border-zinc-800 text-center">
          <div className="text-[10px] font-bold text-zinc-400 uppercase">Pipeline Value</div>
          <div className="text-xl font-black text-rose-500 mt-1">
            {formatCurrency(pipelineSummary.totalPipelineValue, currency)}
          </div>
        </div>
        <div className="bg-zinc-950 p-5 rounded-3xl border border-zinc-800 text-center">
          <div className="text-[10px] font-bold text-zinc-400 uppercase">Total Opportunities</div>
          <div className="text-xl font-black text-white mt-1">{pipelineSummary.totalDeals}</div>
        </div>
        <div className="bg-zinc-950 p-5 rounded-3xl border border-zinc-800 text-center">
          <div className="text-[10px] font-bold text-zinc-400 uppercase">Win Rate</div>
          <div className="text-xl font-black text-emerald-400 mt-1">
            {pipelineSummary.winRatePct.toFixed(1)}%
          </div>
        </div>
        <div className="bg-zinc-950 p-5 rounded-3xl border border-zinc-800 text-center">
          <div className="text-[10px] font-bold text-zinc-400 uppercase">Avg Deal Size</div>
          ...
          return (
            <div key={col.key} className="space-y-3 min-w-[200px]">
              ...
              <div className="p-3 rounded-2xl bg-zinc-950 border border-zinc-800 flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-zinc-500 px-2 py-0.5 rounded-full bg-zinc-900">
                  {stageDeals.length}
                </span>
              </div>
              ...
              <div className="space-y-3 min-h-[300px]">
                {stageDeals.map((c) => (
                  <div
                    key={c.id}
                    className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-zinc-700 transition-all space-y-3 shadow-sm group"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-extrabold px-2.5 py-0.5 rounded-full bg-rose-600/20 text-rose-400 border border-rose-600/30">
                        {c.tags[0] || 'Opportunity'}
                      </span>
                      <button
                        onClick={() => handleDeleteContact(c.id)}
                        className="text-zinc-600 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                    ...
                    <div>
                      <h4 className="font-bold text-white text-xs truncate">{c.company}</h4>
                      <p className="text-[10px] text-zinc-400 mt-0.5 truncate">{c.name}</p>
                    </div>
                    ...
                    <div className="pt-2 border-t border-zinc-900">
                      <select
                        ...
                        className="w-full bg-zinc-900 border border-zinc-800 text-[10px] text-zinc-300 font-bold rounded-lg px-2 py-1 focus:outline-none"
                      >
                        {KANBAN_STAGES.map((s) => (
                ...
                {stageDeals.length === 0 && (
                  <div className="p-6 text-center border border-dashed border-zinc-900 rounded-2xl text-[11px] text-zinc-600">
                    Empty Stage
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-zinc-950 text-white rounded-3xl border border-zinc-800 p-6 max-w-md w-full space-y-4 animate-fadeIn">
            ...
            <form onSubmit={handleCreateLead} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-zinc-400 mb-1">Company Name</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block font-bold text-zinc-400 mb-1">Contact Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-bold text-zinc-400 mb-1">Deal Value ($)</label>
                <input
                  type="number"
                  placeholder="10000"
                  value={newDealValue}
              ...
              <div>
                <label className="block font-bold text-zinc-400 mb-1">Initial Stage</label>
                <select
                  value={newStage}
